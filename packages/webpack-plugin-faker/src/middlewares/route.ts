  ...
  return async function (req: any, res: any, next: any) {
    try {
      ...
      const handleAsset = (
        ...
        if (!filePath) {
          logger.error(
            'Asset path is empty. Please check if dependencies are installed correctly.',
          )
      ...
      const expectedUiPath = path.posix
        .join(publicPath, CLIENT_UI_PATH)
        .replace(/\/+/g, '/')
      const expectedCssPath = path.posix
        .join(publicPath, CLIENT_UI_CSS)
        .replace(/\/+/g, '/')
      const expectedInterceptorPath = path.posix
        .join(publicPath, CLIENT_INTERCEPTOR_PATH)
        .replace(/\/+/g, '/')
